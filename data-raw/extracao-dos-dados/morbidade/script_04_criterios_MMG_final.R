...
arq_obs_cluster <- "data-raw/extracao-dos-dados/morbidade/databases/01_sih_rd/02_arquivos_tratados_long"


# definir diretorio onde estao os arquivos SP

arq_SP <- "data-raw/extracao-dos-dados/morbidade/databases/02_sih_sp"


# Criando um vetor com os anos considerados (2018 a 2023)
anos <- c(2018:2023)
...
for( estado in estados ) {
  ...
  sp_obs_cluster_atual %<>% select( N_AIH, codmunocor = MUNIC_MOV, cnes = CNES, DT_INTER, DT_SAIDA, starts_with( "DIAG" ), starts_with( "CID_" ),
  ...
  sp_obs_cluster_atual %<>% select( N_AIH, codmunocor, cnes, DT_INTER, AIHREF, ORDEM, casos_MMG, casos_MMG_hipertensao,
  ...
  sp_obs_cluster_grp <- sp_uf_obs_clusters %>% group_by( AIHREF ) %>% arrange( ORDEM, .by_group = TRUE ) %>%
    summarise( DT_INTER = first( DT_INTER, order_by = ORDEM ), codmunocor = first( codmunocor, order_by = ORDEM ), cnes = first( cnes, order_by = ORDEM ) )
  ...
  sp_obs_cluster_grp %<>%
    bind_rows( sp_uf_obs_nao_clusters %>%
                 select( AIHREF, DT_INTER, codmunocor, cnes, casos_MMG, casos_MMG_hipertensao, casos_MMG_hemorragia,
  ...
  sp_obs_cluster_grp %<>% mutate( ANO = year( DT_INTER ), MES = month( DT_INTER ) ) %>% filter( between( ANO, anos[1], anos[length(anos)]) )
  ...
  # gerar df com todos os municipios, cnes e os anos
  ## obter todas as combinações únicas de (codmunocor, cnes)
  mun_cnes <- unique(sp_obs_cluster_grp[, c("codmunocor", "cnes")])

  ## construir o grid (codmunocor, cnes, ANO)
  obs_cluster_mun_ano <- tidyr::crossing(
    mun_cnes,
    ANO = anos,
    MES = 1:12
  )

  obs_cluster_mun_ano %<>% arrange(codmunocor, cnes, MES, ANO)


  # juntar df com todos os municipios e os anos com df por municipio e ano

  obs_cluster_mun_ano %<>%
    left_join( sp_obs_cluster_grp %>% group_by(codmunocor, cnes, MES, ANO) %>% # MUDAR AQUI
                 summarise( total_internacoes = n(),
               ...
               by = c( "codmunocor", "cnes", "MES", "ANO" ), na_matches = "never" )
...
# selecionar codmunocor e cnes nao nulos

obs_cluster_total_mun_ano %<>% filter(!is.na(codmunocor) & !is.na(cnes))


# agrupar por codmunocor, cnes e ANO

obs_cluster_total_mun_ano %<>% group_by(codmunocor, cnes, MES, ANO) %>%
...
# salvar as bases
output_dir <- "data-raw/extracao-dos-dados/morbidade/databases/03_bases_finais"
...
write.csv(df_bloco6_morbidade, "data-raw/csv/df_indicadores_morbidade_materna_2018_2023.csv", row.names = FALSE)
